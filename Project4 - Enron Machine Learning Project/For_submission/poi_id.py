...
import os
os.chdir('C:/Users/Rapha/Documents/Datascience_Nanodegree/Projekte/Project4/my_files')
...
# ### Cross-validation
# By making use of the tester.py script provided from Udacity I could run these training and testing runs with minimal code here. What the tester.py script does, it splits the data into features and labels sets with which the classifier is being trained and tested and then randomly shuffles the data before splitting it again into training and testing sets. This process is being repeated several times. In our case 10 times since this is the default value which is also being used in the tester.py script. This iterative shuffling and splitting is one possible form of what is being referred to as cross-validation. 
# 
...
# 
# In the following I will test if feature scaling has a positive effect on the overall algorithm performance.
# 
...
# Using 'Standard Scaler' within a Pipeline produces the exact same results. Therefore I conclude that both methods produce valid results. I assume it will be the case also for the other classifiers...
...
# |-----------------------|----------|-----------|---------|----------|
# | GaussianNB            | 0.80000  | 0.32143   | 0.45000 | 0.37500  |
# | DecisionTree          | 0.83333  | 0.35294   | 0.30000 | 0.32432  |
# | SVC (kernel='linear') | 0.43333  | 0.13483   | 0.60000 | 0.22018  |
# | KMeans (n_clusters=2) | 0.80000  | 0.18750   | 0.15000 | 0.16667  |
# | Adaboost              | 0.82667  | 0.31250   | 0.25000 | 0.27778  |
# | Random Forest         | 0.86667  | 0.50000   | 0.10000 | 0.16667  |
# 
# Scaled:
# 
# | Classifier            | Accuracy | Precision | Recall  | F1 Score |
# |-----------------------|----------|-----------|---------|----------|
# | GaussianNB            | 0.40667  | 0.16505   | 0.85000 | 0.27642  |
# | DecisionTree          | 0.84667  | 0.38462   | 0.25000 | 0.30303  |
# | SVC (kernel='linear') | 0.88667  | 0.80000   | 0.20000 | 0.32000  |
# | KMeans (n_clusters=2) | 0.86667  | 0.50000   | 0.05000 | 0.09091  |
# | Adaboost              | 0.82667  | 0.31250   | 0.25000 | 0.27778  |
# | Random Forest         | 0.88000  | 0.75000   | 0.15000 | 0.25000  |

# The results are mixed. For GaussianNB and DecisionTree the scaled data does not bring an improvement whereas for SVC the performance improved considerably. The performances of Random Forest and KMeans improved a little. AdaBoosts' performance did not change at all though.
...
# KMeans still performes very poorly regarding F1-score so I will drop it for further investigation.
...
# |-----------------------|----------|-----------|---------|----------|
# | GaussianNB            | 0.80000  | 0.32143   | 0.45000 | 0.37500  |
# | DecisionTree          | 0.88000  | 0.54167   | 0.65000 | 0.59091  |
# | SVC (kernel='linear') | 0.43333  | 0.13483   | 0.60000 | 0.22018  |
# | KMeans (n_clusters=2) | 0.87333  | 0.10000   | 0.05000 | 0.09524  |
# | Adaboost              | 0.85333  | 0.44444   | 0.40000 | 0.42105  |
# | Random Forest         | 0.87333  | 0.60000   | 0.15000 | 0.24000  |

# Based on the F1-score the DecisionTree is now leading the pack followed by AdaBoost. GausianNB is on 3rd rank and SVC on 4th.
...
# When tuning the hyperparameters of an estimator, Grid Search and Random Search are both popular methods. According to some publications random search is even perceived as the better method depending on the type of problem. That is why I wanted to see how it performs in direct comparison.
...
# RandomSearch returns a k-value of 16 resulting in a lower F1-score than with the 18 features that GridSearch identified as best. So in my case GridSearch returns a better result.
...
# #### Randomized Search with SelektKBest for AdaBoostClassifier
...
# Also for AdaBoost GridSearchCV delivers better results than RandomSearch. The optimal number of features found by SelectKMeans using GridSearch for the AdaBoostClassifier is 16. 
# 
# Therefore the following SelektKBest for RandomForest and GaussianNB will be run in combination with GridSearch.
...
# Using StandardScaler results in the same score and parameters. So no need to make use of it.
...
# |-------------------|-------------|------------------------|----------------|---------------|
# | __Optimal Value__ |      16     | DecisionTreeClassifier |       0.5      |      50       |
# 
# __Random Forest Classifier__:
# 
# | Parameter         | SelectKBest | criterion | max_depth | n_estimators |
# |------------------ |-------------|-----------|-----------|--------------|
# | __Optimal Value__ |      21     |   'gini'  |     5     |      10      |
# 
# __SVC Classifier__:
# 
...
# In case of the Decision Tree I was trying both scaled and unscaled data throughout the whole process. Reason being that in some out-of-the-box runs the scaled data yielded better performance. But apart from some out-of-the-box runs(which are not shown in this notebook) the scaled data never exceeded the unscaled. At best they were on par.
...
# In case of the SVC it was more advantageous to apply  standard scaling. The positive effect was already visible from the out-of-the-box runs.
...
# |--------------|----------|-----------|--------|----------|
# | DecisionTree | 0.913    | 0.630     | 0.850  | 0.723    |
# | AdaBoost     | 0.873    | 0.522     | 0.600  | 0.558    |
# | RandomForest | 0.847    | 0.286     | 0.100  | 0.148    |
# | SVC          | 0.860    | 0.476     | 0.500  | 0.488    |
# | GaussianNB   | 0.820    | 0.360     | 0.450  | 0.400    |
# 
...
# 
# The weakest performance delivered RandomForest not even reaching the minimum requirements of this project of 0.3 for precision and recall. But since RandomForest is supposed to be very powerful I did not want to give up on it until the final evaluation.
# 
# GaussianNB was strongest in the initial run but without options for parameter tuning it lacked further improvement possibilities.
